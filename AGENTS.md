...
single instruction file. Do not create a duplicate `CLAUDE.md`.
...
- Commit source, docs, tests, and durable `.agentrail/` state together before
  completing a step. Stage named files, not broad worktree sweeps.
- Do not modify `../sw-mlpl` from this repository. Record verified gaps in
  `docs/sw-mlpl-blockers.md`; upstream work requires separate authorization.
- Never install over stable user binaries. Prefer `$MLPL` and `$MLPLUNIT`, then
  documented adjacent development binaries.

## Demo and testing contract

- Use native mlplunit for executable MLPL tests: root `mlplunit.conf`,
  `tests/test_*.mlpl`, `@test` registration, shared assertions, and
  `u:run_registered_tests()`.
- Use a root `justfile` as the human and agent entry point once executable
  checks exist. Keep recipes thin and delegate portable work to `scripts/`.
- Apply TDD. Each lesson is deterministic, standalone, self-checking, and
  updates its tests, catalog, syllabus, and capability ledger together.
- Prefer whole-array operations and small pure functions. Explain any explicit
  loop and record the missing general operation when the loop reveals one.
- A builtin may be a primitive or a correctness oracle, but must not hide the
  mathematics the lesson claims to demonstrate.

## Teaching and visualization contract

- Every lesson teaches a mathematical idea and why it matters to ML. It states
  prerequisites, shapes, the exact executable claim, numerical tolerances, and
  what the finite example does not prove.
- CLI lessons show arrays and numerical evidence before optional artifacts.
  Web lessons are standalone, dependency-free `.mlpl` files with a framed
  `WHAT THIS SHOWS` prolog, explanatory comments beside the code and before
  each visual, and a `HOW TO READ THE RESULT` / `THE POINT` epilog.
- Visuals are evidence, not decoration. Use vectors, grids, transformed basis
  shapes, residual geometry, spectra, or ML data when those make a relationship
  easier to understand. Every visual has a static/textual equivalent.
- Generated artifacts live under `out/`; stable demo-produced previews may be
  committed under `assets/previews/` and must have a freshness check.
- Never claim a numerically approximate equality without a documented
  tolerance, conditioning assumptions, and a failing/counterexample fixture.

## Capability discipline

- Verify capability claims against the configured interpreter, not only docs.
- Classify findings as supported, awkward-but-expressible, or blocking.
- A blocker needs a minimal probe, required signature/semantics, acceptance
  cases, affected lessons, and an honest currently-unavailable status.
- Do not build a large compatibility layer to simulate missing general linear
  algebra. Advance to the next capability-supported lesson and leave the
  blocked lesson queued.

Planning references: `docs/research.txt`, `docs/plan.md`, `docs/syllabus.md`,
`docs/sagas.md`, and `docs/sw-mlpl-blockers.md`.
